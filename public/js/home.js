...
function getWPM() {
  ...
  if (wpm > HighScore) {
    HighScore = wpm;
    document.getElementById("highestScore").innerText = wpm;
  }
}
...
function startTest() {
  inpField.disabled = false;
  const startTestBtn = document.querySelector(".start-typing");
  const tryagainreset = document.querySelector(".try-again");
  tryagainreset.classList.remove("hidden");
  startTestBtn.classList.add("hidden");
  newGame();
}
function toggleRoomsModal() {
  const roomModal = document.querySelector(".roomsModal");
  const overlay = document.querySelector(".overlay");
  roomModal.classList.toggle("hidden");
  overlay.classList.toggle("hidden");
}
const roomsBtn = document.querySelector(".rooms-option");
